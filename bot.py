...
- Instagram checking: via Apify (primary) with RapidAPI fallbacks.
...
# ---------- Instagram status check - COMPLETELY REWRITTEN ----------

async def _check_via_apify(username: str):
    """Check Instagram via Apify - properly handles all error cases."""
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(
                APIFY_URL,
                ...
                # Accept 200 or 201 (Apify returns 201 for sync runs)
                ...
                print(f"[apify] @{username}: response type: {type(data)}", flush=True)
                ...
                # Apify can return errors in many formats
                has_error = False
                error_msg = None
                
                # Check for error fields in the item
                if isinstance(item, dict):
                    ...
                    elif "errorDescription" in item and item["errorDescription"]:
                        has_error = True
                        error_msg = item["errorDescription"]
                    ...
                    elif not item.get("username"):
                        has_error = True
                        error_msg = "no username in response"
                    # Check if followers/posts are None (indicates error)
                    elif item.get("followersCount") is None and item.get("postsCount") is None:
                        has_error = True
                        error_msg = "null data fields"
                
                if has_error:
                    print(f"[apify] @{username}: error: {error_msg}", flush=True)
                    return None
                
                # Extract the data
                result_username = item.get("username") or item.get("user", {}).get("username")
                if not result_username:
                    print(f"[apify] @{username}: no username found in response", flush=True)
                    return None
...
async def _check_via_stable_api(username: str):
    """Check Instagram via stable-api (RapidAPI fallback)."""
    headers = {
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                if resp.status != 200:
                    print(f"[stable-api] @{username}: HTTP {resp.status}", flush=True)
                    return None
                data = await resp.json(content_type=None)
                
                # Check for errors
                if not data:
                    print(f"[stable-api] @{username}: empty response", flush=True)
                    return None
                
                # Check for error field
                if data.get("error") or data.get("message"):
                    print(f"[stable-api] @{username}: error: {data.get('error') or data.get('message')}", flush=True)
                    return None
                
                # Check for username
                if "username" not in data or not data["username"]:
                    print(f"[stable-api] @{username}: no username", flush=True)
                    return None
        ...
        print(f"[stable-api] @{username}: TIMEOUT", flush=True)
        return None
    except Exception as e:
        print(f"[stable-api] @{username}: exception: {type(e).__name__}: {e}", flush=True)
        return None


async def _check_via_instagram120(username: str):
    """Check Instagram via instagram120 (final fallback)."""
    headers = {
        "Content-Type": "application/json",
...
async def check_instagram_status(username: str):
    """Check Instagram status using multiple providers."""
    ...
    # Try stable-api
    print(f"🔄 @{username}: trying stable-api...", flush=True)
    info = await _check_via_stable_api(username)
    if info is not None:
        print(f"✅ @{username} found via stable-api", flush=True)
        return info
    
    # Try instagram120
    print(f"🔄 @{username}: trying instagram120...", flush=True)
        ...
        for username, meta in tracked.items():
            ...
            if track_type == "ban":
                if info is None:
                    ...
                    # Account is reachable - reset fail count
