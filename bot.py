    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                # CRITICAL: Apify returns error responses that still contain a
                # "username" field (e.g. {"username": "zoroxbot", "error": "not_found"}).
                # Check for an error field before accepting the response as valid.
                if "error" in item:
                    print(f"[apify] @{username}: API returned error: {item.get('error')}", flush=True)
                    return None
            ...
            if track_type == "ban":
                if info is None:
                    ...
                    if fail_count >= 2:
                        ...
                        if ok:
                            print(f"🚫 Banned (confirmed after {fail_count} checks): @{username}", flush=True)
                        else:
