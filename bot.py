    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                # not 200 — accept both 200 and 201 as success codes.
                ...
                # CRITICAL: Apify returns error responses in multiple formats:
                # 1. Direct "error" field: {"username": "zoroxbot", "error": "not_found"}
                # 2. Error in nested response: {"username": "zoroxbot", "errorDescription": "Post does not exist"}
                # 3. Error in the response structure itself
                # Check for any error indicators before accepting the response.
                if "error" in item or "errorDescription" in item:
                    error_msg = item.get("error") or item.get("errorDescription", "unknown error")
                    print(f"[apify] @{username}: API returned error: {error_msg}", flush=True)
                    return None
...
async def check_tracked_accounts():
    try:
        ...
        for username, meta in tracked.items():
            ...
            # Skip already-finished entries — no point burning an API call
            # on an account that's already been marked banned/recovered and
            # will never notify again anyway.
            ...
            if track_type == "ban":
                if info is None:
                    # Require 2 consecutive unreachable checks before
                    # declaring a real ban — a single miss can be a
                    # transient API hiccup, not proof of an actual ban.
                    ...
                    if fail_count >= 2:
                        ...
                        if ok:
                            print(f"✅ Banned notification sent and saved for @{username}", flush=True)
                        else:
                            print(f"⚠️ Sent ban notification for @{username} but failed to save to database — will re-notify next check.", flush=True)
                    else:
                ...
                if info is not None and not meta.get("recovered", False):
                    ...
                    if ok:
                        print(f"✅ Recovery notification sent and saved for @{username}", flush=True)
                    else:
                        print(f"⚠️ Sent recovery notification for @{username} but failed to save to database — will re-notify next check.", flush=True)
...
async def checknow(interaction: discord.Interaction, username: str):
    ...
    if info:
        await interaction.followup.send(
            f"✅ **@{info['username']}** is LIVE — Followers: {info['followers']:,} | Posts: {info['posts']:,}"
        )
    else:
        await interaction.followup.send(
...
async def track(interaction: discord.Interaction, username: str):
    ...
    if ok:
        ...
        await interaction.response.send_message(
            f"❌ Failed to start tracking @{username} — the database write did not succeed.",
            ephemeral=True,
        )
...
async def untrack(interaction: discord.Interaction, username: str):
    ...
    if username in tracked:
        ...
        if ok:
            ...
            await interaction.response.send_message(
                f"❌ Failed to remove @{username} — the database delete did not succeed.",
                ephemeral=True,
            )
    else:
...
async def ban(interaction: discord.Interaction, username: str):
    ...
    if ok:
        ...
        await interaction.response.send_message(
            f"❌ Failed to start monitoring @{username} — the database write did not succeed.",
            ephemeral=True,
        )
